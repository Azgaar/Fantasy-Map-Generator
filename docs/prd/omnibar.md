...
The `feat/omnibar-assistant` integration extends this palette with an expanding assistant conversation. See [Omnibar assistant](omnibar-assistant.md) for query submission, context, lifecycle and provider behaviour.
...
- An empty normal query shows only recent commands. With no history, show the empty input and the Assistant entry point. A bare `>` lists configured commands, with recent commands first.
...
- The TypeScript controller in `src/controllers/omnibar.ts` contains a non-exported class and a single exported omnibar instance. Its public methods are `open` and `close`; `open({assistant: true})` reaches the conversation directly. Everything else — dismissal, availability, ranking, rendering — stays private, and tests drive the palette through those two methods plus the rendered input and list. It is registered in the lazy controller registry and loaded on first use, so nothing of it sits in the initial bundle.
...
- Unrestricted natural-language command execution, semantic/vector indexing, or a generic automation API. Bounded assistant questions are covered by the integration specification.
