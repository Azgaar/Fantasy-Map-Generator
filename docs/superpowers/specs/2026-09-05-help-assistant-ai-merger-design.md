...
- [x] Quill branch merged; bridge re-implemented over Quill; scenarios rerun — fork main (1.151.2 sync, PR #68) merged first so the Quill base was an ancestor; `feat/notes-editor-quill` cca5c040 merged with only stamp/notes-editor/rich-text conflicts; bridge sits on their `loadNote`/`updateNotesBox`, selection via `getSemanticHTML(range)`; real `npm ci` clean; 102 files / 1066 unit tests; 4/4 browser scenarios; second merge of the final branch (8991dabc: tinymce folder deleted, leading-script guard, e2e, `&nbsp;` fix, bump to 1.151.3) done — only the version fields conflicted; package.json/lock follow `versioning.ts` to 1.151.3 (`sync-version --check` passes); `getSelectionHtml` strips `&nbsp;` like their save path; `th` dropped from the tool's tag list; 102 files / 1068 unit tests; 7/7 e2e (my 4 + their 3)
