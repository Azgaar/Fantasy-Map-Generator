...
* **Feature** — the name. Islands use the culture of their first land cell, and lakes use a shore cell, with a one-in-ten chance of an English adjective instead. Oceans use an adjective or the side of the map they lie on (_Azure_, _Northern_) — the subtype (_Sea_, _Gulf_) is shown in its own column. Compatible older maps receive missing names on load. Type a name to change it; clear the field to make it _Unnamed_
