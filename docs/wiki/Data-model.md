<!-- MIRRORED PAGE: do not edit here. The source of truth is docs/architecture/data_model.md in the main repository; update that file and copy it over (a sync step can automate this). -->

**FMG data model** is poorly defined, inconsistent and not well-documented. This page is an attempt to document it. Once everything is documented, it can be used for building a new consistent model. Please note the current document reflect the object model **as is**, so with all its quirks. The model we want to get is covered in the [future data model](https://github.com/Azgaar/Fantasy-Map-Generator/blob/master/docs/architecture/future_data_model.md) page.
