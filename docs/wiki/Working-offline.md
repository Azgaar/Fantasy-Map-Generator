**Fantasy Map Generator** is a web-tool, but it supports working offline.
...
* Additional Fonts are not available (fetched live from Google Fonts).
* Additional Textures are not available.

A desktop app build (via Electron) is in development but not yet released.

## Manual/developer option

If you'd rather run your own local copy &mdash; for development, or to run a specific version &mdash; see [Run FMG locally](Run-FMG-locally).
