/**
 * Japanese UI: applies after ja-tip-map.js (tooltips) and before general.js.
 */
(function () {
  "use strict";
  ...
  function applyHeadAndLoading() {
    document.documentElement.lang = "ja";
    document.title = "Azgaar 空想マップジェネレータ";
    const desc =
      "ファンタジー作家、ゲームマスター、地図制作者向けの無料Webアプリ。空想地図の作成と編集ができます。";
    const setMeta = (sel, content) => {
      const n = document.querySelector(sel);
      if (n) n.setAttribute("content", content);
    };
    setMeta('meta[name="application-name"]', "Azgaar 空想マップジェネレータ");
    setMeta('meta[name="description"]', desc);
    setMeta('meta[property="og:title"]', "Azgaar 空想マップジェネレータ");
    setMeta('meta[property="og:description"]', desc);
    setMeta('meta[name="twitter:title"]', "Azgaar 空想マップジェネレータ");
    setMeta('meta[name="twitter:description"]', desc);

    setText("titleName", "Azgaar's");
    setText("title", "空想マップジェネレータ");
    const lt = document.getElementById("loading-text");
    if (lt) {
      lt.innerHTML = "読み込み中<span>.</span><span>.</span><span>.</span>";
    }
  }

  function applyTabsAndMainButtons() {
    setText("layersTab", "レイヤー");
    setText("styleTab", "スタイル");
    setText("optionsTab", "オプション");
    setText("toolsTab", "ツール");
    setText("aboutTab", "情報");
    setText("regenerate", "新しいマップ");
    setText("configureWorld", "世界を設定");
    setText("optionsReset", "既定に戻す");
  }
  ...
  function applyOptionsBits() {
    const loadBtn = document.getElementById("loadGoogleTranslateButton");
    if (loadBtn) loadBtn.textContent = "Google翻訳を読み込む";
    document.querySelectorAll("#optionsContent td").forEach(td => {
      if (td.textContent.trim() === "Language") td.textContent = "言語";
    });
  }
  ...
  function applyCustomizationMenu() {
    const menu = document.getElementById("customizationMenu");
    if (!menu) return;
    const ps = menu.querySelectorAll(":scope > p");
    if (ps[0]) ps[0].textContent = "ハイトマップ調整ツール:";
    if (ps[1]) ps[1].textContent = "オプション:";
    setText("paintBrushes", "ペイントブラシ");
    setText("applyTemplate", "テンプレート");
    setText("convertImage", "画像変換");
    setText("heightmapPreview", "プレビュー");
    setText("heightmap3DView", "3D表示");
  }

  function applyJapaneseUI() {
    applyHeadAndLoading();
    applyTabsAndMainButtons();
    applyLayersTab();
    applyToolsTab();
    applyOptionsBits();
    applyStyleTabBits();
    applyCustomizationMenu();
    applyDataTips();
  }

  if (document.readyState === "loading") {
    document.addEventListener("DOMContentLoaded", applyJapaneseUI);
  } else {
    applyJapaneseUI();
  }
})();
