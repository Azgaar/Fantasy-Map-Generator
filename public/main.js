...
// after Layers.init() creates the groups, and again by load.ts once a loaded map replaces the SVG.
//
// Uses the GLOBAL d3 (v5) deliberately: legacy zoom/mouse handlers rely on v5 setting `d3.event`,
// and a v7 selection dispatches without it, breaking mouse and zoom after a load (#1508).
const ELEMENT_GLOBALS = ["airroutes", "anchors", "armies", "biomes", "borders", "burgIcons", "cells", "coastline", "compass", "coordinates", "cults", "debug", "emblems", "fogging", "goods", "gridOverlay", "ice", "icons", "labels", "lakes", "landmass", "legend", "markers", "markets", "ocean", "oceanLayers", "oceanPattern", "population", "prec", "provinceBorders", "provs", "regions", "relig", "rivers", "roads", "routes", "ruler", "scaleBar", "searoutes", "stateBorders", "statesBody", "statesHalo", "temperature", "terrain", "terrs", "texture", "tradeAnimation", "traderoutes", "trails", "viewbox", "zones"];
function bindElementGlobals() {
  for (const id of ELEMENT_GLOBALS) window[id] = d3.select("#" + id);
  window.svg = d3.select("#map");
  window.defs = d3.select("#deftemp");
}
window.bindElementGlobals = bindElementGlobals;
bindElementGlobals();
