...
window.ThreeDErosion = (function () {
  ...
  function getBakeSize(bakeResolution) {
  ...
  function buildCoastTexture(bakeW, bakeH) {
    const scaleX = bakeW / graphWidth;
    const scaleY = bakeH / graphHeight;
    const isLand = feature => feature && feature.type !== "ocean" && feature.type !== "lake";
  ...
  function buildRiverCanvas(bakeW, bakeH) {
  ...
  // One pass, evaluated per pixel in isolation. uv space: (0,0) = map top-left.
  // Heights are in the normalized 0..1 == h 0..100 domain throughout
  const fragmentShader = /* glsl */ `
    ...
    void main() {
      ...
      h += detail * RIDGE_SHARPEN * hillCurve;
      ...
      gl_FragColor = vec4(pack16(h), 1.0 - landFactor, 1.0);
    }
  `;

  function runErosionPass(renderer, params, bakeW, bakeH, textures) {
  ...
  // bake() returns the cached field when nothing relevant changed.
  // readRenderTargetPixels returns rows bottom-up, which matches uv.y = 0 at
  // gl_FragCoord.y = 0 = map top: row 0 of the decoded field is the map top row
  async function bake(renderer, params) {
    ...
    try {
      ...
      const textures = {
        ...
        coast: buildCoastTexture(bakeW, bakeH),
      ...
      cached = { key, heights, cols: bakeW, rows: bakeH };
  ...
  // three r140 has no BVH and stalls on ~1M triangles)
  function heightAt(x, y, scale) {
    if (!cached) return 0;
    const { heights, cols, rows } = cached;
    ...
    const top = heights[y0 * cols + x0] * (1 - tx) + heights[y0 * cols + x1] * tx;
    const bottom = heights[y1 * cols + x0] * (1 - tx) + heights[y1 * cols + x1] * tx;
    const h = top * (1 - ty) + bottom * ty;
  ...
  return { bake, heightAt, makeKey, isCached, dispose };
})();
