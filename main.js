...
// Switches to disable/enable logging levels
var DEBUG = 0;
var WARN = 0;
var ERROR = 0;
...
void function checkLoadParameters() {
  ...
  if (params.get("maplink")) {
    WARN && console.error("Load map from URL");
  ...
  if (params.get("seed")) {
    WARN && console.error("Generate map for seed");
    generateMapOnLoad();
    return;
  }
  ...
  if (onloadMap.value === "saved") {
    ldb.get("lastMap", blob => {
      if (blob) {
        WARN && console.error("Load last saved map");
        try {
          uploadMap(blob);
        }
        catch(error) {
          ERROR && console.error(error);
          WARN && console.error("Cannot load stored map, random map to be generated");
          generateMapOnLoad();
        }
      } else {
  ...
  WARN && console.error("Generate random map");
  generateMapOnLoad();
}()
...
function generate() {
  try {
    ...
    invokeActiveZooming();
    generateSeed();
    DEBUG && console.group("Generated Map " + seed);
    applyMapSize();
    randomizeOptions();
    placePoints();
    ...
    WARN && console.error(`TOTAL: ${rn((performance.now()-timeStart)/1000,2)}s`);
    showStatistics();
    DEBUG && console.groupEnd("Generated Map " + seed);
  }
  catch(error) {
    ERROR && console.error(error);
    clearMainTip();
...
function placePoints() {
  DEBUG && console.time("placePoints");
  ...
  DEBUG && console.timeEnd("placePoints");
}

// calculate Delaunay and then Voronoi diagram
function calculateVoronoi(graph, points) {
  DEBUG && console.time("calculateDelaunay");
  const n = points.length;
  const allPoints = points.concat(grid.boundary);
  const delaunay = Delaunator.from(allPoints);
  DEBUG && console.timeEnd("calculateDelaunay");

  DEBUG && console.time("calculateVoronoi");
  ...
  DEBUG && console.timeEnd("calculateVoronoi");
}

// Mark features (ocean, lakes, islands)
function markFeatures() {
  DEBUG && console.time("markFeatures");
  ...
  DEBUG && console.timeEnd("markFeatures");
}
...
function openNearSeaLakes() {
  ...
  DEBUG && console.timeEnd("openLakes");
}
...
function calculateTemperatures() {
  DEBUG && console.time('calculateTemperatures');
  ...
  DEBUG && console.timeEnd('calculateTemperatures');
}

// simplest precipitation model
function generatePrecipitation() {
  DEBUG && console.time('generatePrecipitation');
  ...
  DEBUG && console.timeEnd('generatePrecipitation');
}

// recalculate Voronoi Graph to pack cells
function reGraph() {
  DEBUG && console.time("reGraph");
  ...
  DEBUG && console.timeEnd("reGraph");
}

// Detect and draw the coasline
function drawCoastline() {
  DEBUG && console.time('drawCoastline');
  reMarkFeatures();
  ...
  DEBUG && console.timeEnd('drawCoastline');
}

// Re-mark features (ocean, lakes, islands)
function reMarkFeatures() {
  DEBUG && console.time("reMarkFeatures");
  ...
  DEBUG && console.timeEnd("reMarkFeatures");
}
...
function elevateLakes() {
  if (templateInput.value === "Atoll") return; // no need for Atolls
  DEBUG && console.time('elevateLakes');
  ...
  DEBUG && console.timeEnd('elevateLakes');
}

// assign biome id for each cell
function defineBiomes() {
  DEBUG && console.time("defineBiomes");
  ...
  DEBUG && console.timeEnd("defineBiomes");
}
...
function rankCells() {
  ...
  DEBUG && console.timeEnd('rankCells');
}

// generate some markers
function addMarkers(number = 1) {
  if (!number) return;
  DEBUG && console.time("addMarkers");
  ...
  DEBUG && console.timeEnd("addMarkers");
}

// regenerate some zones
function addZones(number = 1) {
  DEBUG && console.time("addZones");
  ...
  DEBUG && console.timeEnd("addZones");
}
...
const regenerateMap = debounce(function() {
  WARN && console.error("Generate new random map");
