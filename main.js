...
function applyDefaultBiomesSystem() {
    ...
    return {i: d3.range(0, biomeName.length), name: biomeName, color: biomeColor, biomesMartix, habitability, iconsDensity, icons, cost};
}
...
function rankCells() {
    ...
    for (const i of cells.i) {
        ...
        if (flMean) s += normalize(cells.fl[i] + cells.conf[i], flMean, flMax) * 500; // big rivers and confluences are valued
        s -= (cells.h[i] - 50) / 5; // low elevation is valued, high is not;

        if (cells.t[i] === 1) { // What is t?
            if (cells.r[i]) s += 5; // estuary is valued
            const feature = features[cells.f[cells.haven[i]]];
            if (feature.type === "lake") {
                if (feature.group === "freshwater") s += 400;
                else if (feature.group == "salt") s -= 5;
                else if (feature.group == "frozen") s += 1;
                else if (feature.group == "dry") s -= 2;
                else if (feature.group == "sinkhole") s += 3;
                else if (feature.group == "lava") s += 30;
            } else {
                s -= 100; // ocean coast is valued
                if (cells.harbor[i] === 1) s -= 10; // safe sea harbor is valued
            }
        }

        cells.s[i] = s / 50; // general population rate
