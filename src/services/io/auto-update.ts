...
export async function resolveVersionConflicts(mapVersion: string, data: string[]): Promise<void> {
  ...
  if (isOlderThan("1.109.0")) {
    ...
    select("#burgIcons")
      ...
      .each(function (_el, index) {
        ...
        options.map.burgs.groups.push({ name, active: true, order: index + 1, isDefault, preview: "settlemaker" });
    ...
    validBurgs.forEach(burg => {
      if (!burg.group) Burgs.defineGroup(burg, popIndex, populations.length);
  ...
  if (isOlderThan("1.140.0")) {
    ...
    for (const burgGroup of options.map.burgs.groups) {
    ...
    for (const burg of pack.burgs) {
      ...
      delete (burg as { labelDx?: number }).labelDx;
      delete (burg as { labelDy?: number }).labelDy;
    }
