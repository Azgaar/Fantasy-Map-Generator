...
export function resolveVersionConflicts(mapVersion: string, data: string[]): void {
  ...
  if (isOlderThan("1.140.0")) {
    ...
    // migrate legacy Province label text and offsets; paths are regenerated by the shared region-label renderer
    for (const textEl of document.querySelectorAll<SVGTextElement>(
      "#provs #provinceLabels > text[id^='provinceLabel']"
    )) {
