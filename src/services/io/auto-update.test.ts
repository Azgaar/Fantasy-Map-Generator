...
it.each([18, 180])("keeps legacy custom labels after saving and reloading a font size of %s", async fontSize => {
  ...
  // the fork derives bounds the Label Groups editor accepts: null when unbounded, else >= 0.01
  expect(group?.zoom.min ?? 0.01).toBeGreaterThanOrEqual(0.01);
  expect(group?.zoom.max ?? 0.01).toBeGreaterThanOrEqual(0.01);
...
describe("v1.140 label group migration", () => {
  ...
  function setupLegacyMap() {
    ...
    options.map.labels.groups = [];
    options.map.burgs.groups = LEGACY_BURG_GROUPS.map(({ id }) => ({ name: id })) as never;
  ...
  it("derives zoom bounds the Label Groups editor accepts", () => {
    ...
    const outOfRange = options.map.labels.groups
    ...
    expect(options.map.labels.groups.find(({ name }) => name === "state")?.zoom).toEqual({ min: null, max: 2.4 });
    // a legacy burg group named like a modern one takes the modern group's bounds (1.150 migration)
    expect(options.map.labels.groups.find(({ name }) => name === "towns")?.zoom).toEqual({ min: 2, max: 30 });
  });
  ...
  it("renames a legacy group that claims a name the migration reserves", () => {
    ...
    const names = options.map.labels.groups.map(({ name }) => name);
    expect(names).toHaveLength(new Set(names).size);
    expect(options.map.labels.groups.find(({ name }) => name === "river")?.type).toBe("river");
    expect(options.map.labels.groups.find(({ name }) => name === "river2")?.type).toBe("burg");
