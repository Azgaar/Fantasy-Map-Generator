...
export class ProductionChains {
  ...
  static open() {
    ...
    const dialogSize = ProductionChains.getDialogSize(bounds);
    const graphMarkup = ProductionChains.renderGraph(layout);
    ...
    if (svgEl) ProductionChains.attachGraphInteractions(svgEl, layout);
  ...
  private static getChainGoods(goods: Good[]): Good[] {
  ...
  private static createNodesForComponent(
  ...
  private static renderFlowAnimationStyle(): string {
    return `<style>
  ...
  private static getEdgeStrokeColor(index: number): string {
    return Goods.getStroke(C_12[index % C_12.length]);
  }

  private static getEdgeColorIndex(displayEdge: DisplayEdge): number {
  ...
  private static getDisplayEdges(edges: RoutedEdge[]): DisplayEdge[] {
  ...
  private static renderEdge(displayEdge: DisplayEdge, positions: Map<number, Position>): string {
    ...
    ${flows}
    ${ProductionChains.renderEdgePath(geometry, flowColor, `ca${edgeColorIndex}`)}
    ${labels}
  </g>`;
  }

  private static renderNodeTooltip(node: GraphNode): string {
    return [
  ...
  private static renderNodeFrame(node: GraphNode, stroke: string): string {
  ...
  private static renderNode(node: GraphNode, positions: Map<number, Position>): string {
    ...
    <title>${tooltip}</title>
    ${ProductionChains.renderNodeFrame(node, stroke)}
    ${ProductionChains.renderNodeContent(node, displayName)}
  </g>`;
  }
  ...
  private static renderGraph(layout: LayoutData): string {
    const positions = ProductionChains.getBasePositions(layout.nodes);
    ...
    <defs>${ProductionChains.renderMarkers()}</defs>
    ${ProductionChains.renderFlowAnimationStyle()}
