...
describe("riversAdapter", () => {
  ...
  it("describeCascade counts deletable rivers and never skips (no lock)", () => {
    const summary = adapter.describeCascade([1, 2, 99]); // 99 missing
    expect(summary.deletable).toBe(2);
    expect(summary.skippedLocked).toBe(0);
    expect(summary.lines.join(" ").includes("2 rivers")).toBe(true);
  });
