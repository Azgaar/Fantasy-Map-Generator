...
function renderRiversPage(view: TableView<River>): void {
  ...
  for (const r of view.rows) {
    ...
    const length = `${rn(r.length * facts.units.distance.scale)} ${unit}`;
    const width = `${rn(r.width * facts.units.distance.scale, 3)} ${unit}`;
  ...
  ensureEl("riversFooterLength").innerHTML = `${averageLength * facts.units.distance.scale} ${unit}`;
  const averageWidth = rn(mean(view.all.map(r => r.width))!, 3) || 0;
  ensureEl("riversFooterWidth").innerHTML = `${rn(averageWidth * facts.units.distance.scale, 3)} ${unit}`;
...
function downloadRiversData(): void {
  ...
  exported.forEach((r: River) => {
    const discharge = `${r.discharge} m³/s`;
    const length = `${rn(r.length * facts.units.distance.scale)} ${facts.units.distance.unit}`;
    const width = `${rn(r.width * facts.units.distance.scale, 3)} ${facts.units.distance.unit}`;
    const basin = riversById.get(r.basin)?.name || "";
