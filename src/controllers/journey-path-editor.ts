...
function onDragControlPoint(event: any): void {
  ...
  event.on("drag", function (this: SVGCircleElement, dragEvent: any) {
    ...
    const cellId = Pack.findCell(x, y) ?? original[2];
  ...
  event.on("end", () => {
    if (isPointAllowed(droppedCellId, domain, isEndpoint)) {
      seg.points = resampleAround(seg.points, pointIndex, pointEditSpacing, (x, y) => Pack.findCell(x, y));
      seg.distance = pathLength(seg.points);
    } else {
...
function onAddControlPoint(this: SVGPathElement, event: MouseEvent): void {
  ...
  const px = rn(x, 2);
  const py = rn(y, 2);
  const cellId = Pack.findCell(px, py);
  if (cellId === undefined) return;
...
function showDrawToolbar(): void {
  ...
  ensureEl("journeyDrawFinish").addEventListener("click", finishCustomPath);
  ensureEl("journeyDrawUndo").addEventListener("click", undoLastCustomPoint);
  ensureEl("journeyDrawCancel").addEventListener("click", () => {
    cancelCustomPath();
    host?.refresh();
  });
}
