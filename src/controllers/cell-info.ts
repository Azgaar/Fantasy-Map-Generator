...
function updateFields(point: Point, cellId: number, gridCellId: number): void {
  ...
  const latitude = getLatitude(y, options.map.geography.coordinates, options.map.graph.height, 4);
  set("infoLat", toDMS(latitude, "lat"));
  set("infoLon", toDMS(getLongitude(x, options.map.geography.coordinates, options.map.graph.width, 4), "lon"));
...
function getElevation(feature: Feature, h: number): string {
  ...
  if (feature.border) return `0 ${options.map.units.height.unit}`; // ocean: 0
...
function getDepth(feature: Feature, [x, y]: Point): string {
  if (feature.land) return `0 ${options.map.units.height.unit}`; // land: 0
