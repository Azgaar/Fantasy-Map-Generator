...
const panel = vi.hoisted(() => ({
  mountMapPanel: vi.fn((host: HTMLElement) => {
    host.innerHTML = '<textarea id="helpMapInput"></textarea><button id="helpMapSettings">Settings</button>';
  }),
...
import { getSelection, setSelection } from "@/services/agent/map-tools";
...
function rows(): HTMLElement[] {
  return Array.from(document.querySelectorAll<HTMLElement>('#omnibar-list [role=option]:not([data-kind="assistant"])'));
}
...
describe("Omnibar public behavior", () => {
  ...
  it("counts the matches the cap hides", () => {
    ...
    expect(document.getElementById("omnibar-status")?.textContent).toMatch(/^50 of \d+ local results/);
  });
  ...
  it("navigates results with arrows and keeps Tab inside the palette", () => {
    ...
    key("Tab");
    document.getElementById("omnibar-assistant")!.focus();
...
describe("assistant in the omnibar", () => {
  it("keeps typing local and submits the original question only when the assistant result is activated", async () => {
    ...
    Omnibar.open();
    search("Which states have no ports?");
    expect(rows()).toHaveLength(0);
    expect(panel.send).not.toHaveBeenCalled();
    expect(fetch).not.toHaveBeenCalled();
    key("Enter");
    await vi.waitFor(() => expect(panel.send).toHaveBeenCalledOnce());
    expect(document.querySelector<HTMLTextAreaElement>("#helpMapInput")!.value).toBe("Which states have no ports?");
    expect(document.getElementById("helpAssistant")!.closest("#omnibar")).not.toBeNull();
    expect(document.querySelector<HTMLElement>(".omnibar-search")!.hidden).toBe(true);
    expect(document.querySelector(".ui-dialog")).toBeNull();
  });

  it("keeps a matching feature ahead of the assistant action and never sends a command query", () => {
    Omnibar.open();
    search("Aldor");
    expect(input().getAttribute("aria-activedescendant")).toBe(rows()[0].id);
    expect(document.querySelector('[data-kind="assistant"]')).not.toBeNull();
    search("> zoom");
    expect(document.querySelector('[data-kind="assistant"]')).toBeNull();
    expect(panel.send).not.toHaveBeenCalled();
  });

  it("attaches a selected entity without submitting a question or copying the map", async () => {
    Omnibar.open();
    search("Aldor");
    document.getElementById("omnibar-ask-target")!.click();
    await vi.waitFor(() => expect(getSelection()?.target).toBe("burg:1"));
    expect(getSelection()).toEqual({ target: "burg:1", label: "Aldor", mapId: expect.any(String) });
    expect(panel.send).not.toHaveBeenCalled();
    expect(mocks.zoom).not.toHaveBeenCalled();
  });

  it("preserves a draft across search and dismissal, then restores the same conversation panel", async () => {
    await Omnibar.open({ assistant: true });
    const composer = document.querySelector<HTMLTextAreaElement>("#helpMapInput")!;
    composer.value = "Make this note more ominous";
    document.querySelector<HTMLButtonElement>('[data-chat="search"]')!.click();
    expect(document.getElementById("helpAssistant")!.hidden).toBe(true);
    Omnibar.close();
    expect(document.getElementById("omnibar")!.hidden).toBe(true);
    await Omnibar.open({ assistant: true });
    expect(document.getElementById("helpMapInput")).toBe(composer);
    expect(composer.value).toBe("Make this note more ominous");
    expect(panel.mountMapPanel).toHaveBeenCalledOnce();
    expect(panel.send).not.toHaveBeenCalled();
  });

  it("lets conversation controls own Enter and Tab, and closes only the assistant on Escape", async () => {
    await Omnibar.open({ assistant: true });
    const composer = document.querySelector<HTMLTextAreaElement>("#helpMapInput")!;
    const enter = vi.fn();
    composer.addEventListener("keydown", enter);
    composer.dispatchEvent(new KeyboardEvent("keydown", { code: "Enter", key: "Enter", bubbles: true }));
    expect(enter).toHaveBeenCalledOnce();
    const settings = document.getElementById("helpMapSettings")!;
    settings.focus();
    expect(document.activeElement).toBe(settings);
    expect(document.getElementById("omnibar")!.hidden).toBe(false);
    expect(document.getElementById("helpAssistant")!.hidden).toBe(false);
    const tab = new KeyboardEvent("keydown", { code: "Tab", key: "Tab", bubbles: true, cancelable: true });
    settings.dispatchEvent(tab);
    expect(tab.defaultPrevented).toBe(true);
    expect(document.activeElement).toBe(document.querySelector('[data-chat="search"]'));
    composer.dispatchEvent(new KeyboardEvent("keydown", { code: "Escape", key: "Escape", bubbles: true }));
    expect(document.getElementById("omnibar")!.hidden).toBe(true);
  });

  it("resets the panel and rejects old context when a new map is loaded", async () => {
    Omnibar.open();
    search("Aldor");
    document.getElementById("omnibar-ask-target")!.click();
    await vi.waitFor(() => expect(getSelection()?.target).toBe("burg:1"));
    const oldComposer = document.getElementById("helpMapInput");
    // Generation can reuse the pack object: the lifecycle signal must invalidate context too.
    window.dispatchEvent(new Event("map:generated"));
    expect(getSelection()).toBeUndefined();
    expect(panel.unmountMapPanel).toHaveBeenCalled();
    expect(document.getElementById("helpMapInput")).not.toBe(oldComposer);
  });
});
