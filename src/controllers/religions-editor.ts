...
const columns: EditorColumn<Religion>[] = [
  ...
  },
  {
    ...
    width: "6em",
    sortBy: religion =>
      (religion.rural || 0) * facts.units.population.scale +
      (religion.urban || 0) * facts.units.population.scale * facts.units.population.urbanization.rate
  },
  {
    key: "expansion",
    label: "Expansion",
...
function religionsEditorAddLines(view: TableView<Religion>): void {
  ...
  for (const r of view.all) {
    ...
    totalPopulation += rn(
      (r.rural ?? 0) * facts.units.population.scale +
        (r.urban ?? 0) * facts.units.population.scale * facts.units.population.urbanization.rate
    );
  }

  for (const r of view.rows) {
    const area = getArea(r.area ?? 0);
    const rural = (r.rural ?? 0) * facts.units.population.scale;
    const urban = (r.urban ?? 0) * facts.units.population.scale * facts.units.population.urbanization.rate;
...
function changePopulation(this: HTMLElement): void {
  ...
  const rural = rn((religion.rural ?? 0) * facts.units.population.scale);
  const urban = rn((religion.urban ?? 0) * facts.units.population.scale * facts.units.population.urbanization.rate);
