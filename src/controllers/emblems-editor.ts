...
function openInArmoria(): void {
  ...
  const url = `https://azgaar.github.io/Armoria/?coa=${json}&from=Fantasia`;
  openURL(url);
}
...
async function downloadGallery(): Promise<void> {
  ...
  const fantasia = `<a href="https://github.com/patkepa/fantasia" target="_blank">Fantasia</a>`;
  ...
  const html = /* html */ `<!DOCTYPE html>
    <html>
      ...
      <body>
        ...
        <address>Generated by ${fantasia}. The tool is free, but images may be copyrighted, see ${license}</address>
      </body>
    </html>`;
