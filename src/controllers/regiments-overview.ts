...
function getRegimentColumns(): EditorColumn<RegimentRow>[] {
  const unitColumns: EditorColumn<RegimentRow>[] = facts.military.units.map(unit => ({
...
function renderRegimentsPage(view: TableView<RegimentRow>): void {
  ...
  const unitTotals = Object.fromEntries(
    facts.military.units.map(unit => [unit.name, sum(view.all.map(row => row.regiment.u[unit.name] || 0))])
  );
  ...
  footer.innerHTML = /* html */ `<div style="margin-left:4px">Regiments:&nbsp;${view.all.length}</div>
    ${facts.military.units.map(unit => `<div data-col="${unitColumnKey(unit.name)}" style="margin-left:12px">${capitalize(unit.name)}:&nbsp;${si(unitTotals[unit.name])}</div>`).join("")}
...
function downloadRegimentsData(): void {
  ...
  for (const s of pack.states) {
    ...
    for (const r of s.military) {
      ...
      data += `${r.x},`;
      data += `${r.y},`;
      data += `${getLatitude(r.y, facts.geography.coordinates, facts.graph.height, 2)},`;
      data += `${getLongitude(r.x, facts.geography.coordinates, facts.graph.width, 2)},`;

      data += `${r.bx},`;
      data += `${r.by},`;
      data += `${getLatitude(r.by, facts.geography.coordinates, facts.graph.height, 2)},`;
      data += `${getLongitude(r.bx, facts.geography.coordinates, facts.graph.width, 2)}\n`;
    }
  }
