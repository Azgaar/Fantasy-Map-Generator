import { alertDialog, closeDialogs, confirmationDialog, destroyDialog } from "@/components/dialog/dialog-helpers";
...
import { getJourneyColor, setJourneyColorMode } from "@/renderers/draw-journeys";
...
function renderDialog(journey: Journey): void {
  ...
  ensureEl("journeyColor").addEventListener("input", onColorInput);
  ensureEl("journeyColorReset").addEventListener("click", onColorReset);
  ensureEl("journeyEditStyle").addEventListener("click", () => editStyle("journeys"));
  ...
  ensureEl("journeyExport").addEventListener("click", downloadSegments);
  ensureEl("journeyEditTransport").addEventListener("click", () => void Controllers.TransportTypesEditor.open());
  ensureEl("journeyRemove").addEventListener("click", removeJourney);
}
