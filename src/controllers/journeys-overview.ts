...
import { downloadFile, ensureEl, getFileName, getHoursPerDay, rn } from "@/utils";
...
function renderDialog(): void {
  destroyDialog("journeysOverview");
  ...
  ensureEl("journeysOverviewRefresh").addEventListener("click", addLines);
  ensureEl("journeyCreateNew").addEventListener("click", createNewJourney);
  ensureEl("journeysExport").addEventListener("click", downloadJourneys);
  ...
  ensureEl("journeysRemoveAll").addEventListener("click", removeAllJourneys);
  ensureEl("journeysSearch").addEventListener("input", addLines);
}
...
function addLines(): void {
  ...
  body.querySelectorAll("div > span.icon-target").forEach(el => void el.addEventListener("click", zoomToJourney));
  body.querySelectorAll("div > span.icon-pencil").forEach(el => void el.addEventListener("click", openJourneyEditor));
  body.querySelectorAll("div > span.locks").forEach(el => void el.addEventListener("click", toggleLockStatus));
  body.querySelectorAll("div > span.icon-trash-empty").forEach(el => void el.addEventListener("click", removeOne));
}
