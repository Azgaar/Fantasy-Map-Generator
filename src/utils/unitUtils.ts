...
export const convertTemperature = (temperatureInCelsius: number, targetScale?: TemperatureScale) => {
  const scale = targetScale || (facts.units.temperature.unit as TemperatureScale) || "°C";
...
export function getCellPopulation(cellId: number, pack: PackedGraph): [number, number] {
  const rural = pack.cells.pop[cellId] * facts.units.population.scale;
  const burgId = pack.cells.burg[cellId];
  const urban = burgId
    ? (pack.burgs[burgId].population || 0) * facts.units.population.scale * facts.units.population.urbanization.rate
    : 0;
  return [rural, urban];
}
...
export function getDistanceUnit(): string {
  return facts.units.distance.unit || "km";
}
...
export function getAreaUnit(squareMark = "²"): string {
  if (facts.units.area.unit !== "square") return facts.units.area.unit;
  return facts.units.distance.unit + squareMark;
}
...
export function getArea(rawArea: number): number {
  return rawArea * facts.units.distance.scale ** 2;
}
