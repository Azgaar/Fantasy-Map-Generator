...
class StatesModule {
  ...
  private createStates(): State[] {
    const states: State[] = [{ i: 0, name: "Neutrals" }];
  ...
  expandStates() {
    ...
    const queue = new (window as any).FlatQueue();
    const cost: number[] = [];
  ...
  private getBiomeCost(b: number, biome: number, type: string): number {
  ...
  private getHeightCost(f: any, h: number, type: string): number {
  ...
  private getRiverCost(r: number, i: number, type: string): number {
    if (type === "River") return r ? 0 : 100; // penalty for river cultures
    if (!r) return 0; // no penalty for others if there is no river
    return minmax(pack.cells.fl[i] / 10, 20, 100); // river penalty from 20 to 100 based on flux
  }

  private getTypeCost(t: number, type: string): number {
    if (t === 1)
  ...
  normalize() {
    ...
    for (const i of cells.i) {
      ...
      if (cells.c[i].some((c: number) => burgs[cells.burg[c]]?.capital))
        continue; // do not overwrite near capital
      const neibs = cells.c[i].filter((c: number) => cells.h[c] >= 20);
  ...
  findNeighbors() {
    ...
    states.forEach((s) => {
      if (s.removed) return;
      (s as any).neighbors = new Set<number>();
    });
    ...
    for (const i of cells.i) {
      ...
      for (const c of cells.c[i]) {
        if (cells.h[c] >= 20 && cells.state[c] !== s) {
          (states[s] as any).neighbors.add(cells.state[c]);
        }
      }
    }
    ...
    states.forEach((s) => {
      ...
      s.neighbors = Array.from((s as any).neighbors);
    });
  }
  ...
  generateDiplomacy() {
    ...
    const { cells, states } = pack;
    (states[0] as any).diplomacy = [];
    const chronicle: string[][] = (states[0] as any).diplomacy;
    const valid = states.filter((s) => s.i && !s.removed);

    const neibs: Record<DiplomacyStatus, number> = {
      Ally: 1,
      Friendly: 2,
      Neutral: 1,
      Suspicion: 10,
    ...
    const neibsOfNeibs: Record<string, number> = {
      Ally: 10,
      Friendly: 8,
      Neutral: 5,
      Suspicion: 1,
    }; // relations to neighbors of neighbors
    ...
    const navals: Record<string, number> = {
      Neutral: 1,
      Suspicion: 2,
      Rival: 1,
      Unknown: 1,
    }; // relations of naval powers

    // clear all relationships
    for (const s of valid) {
      s.diplomacy = new Array(states.length).fill("x");
    }
    if (valid.length < 2) return; // no states to generate relations with
    const areaMean = (window as any).d3.mean(valid.map((s) => s.area)); // average state area
    ...
    for (let attacker = 1; attacker < states.length; attacker++) {
      ...
      for (const a of attackers) {
        for (const d of defenders) {
          states[a].diplomacy![d] = "Enemy";
          states[d].diplomacy![a] = "Enemy";
        }
      }
  ...
  defineStateForms(list?: number[]) {
    ...
    const theocracy: Record<string, number> = {
      Theocracy: 20,
      Brotherhood: 1,
      Thearchy: 2,
      See: 1,
    ...
    const anarchy: Record<string, number> = {
      "Free Territory": 2,
      Council: 3,
      Commune: 1,
      Community: 1,
  ...
  private selectForm(
    ...
    tier: number,
    monarchy: string[],
    republic: Record<string, number>,
    union: Record<string, number>,
    theocracy: Record<string, number>,
    anarchy: Record<string, number>,
  ): string {
