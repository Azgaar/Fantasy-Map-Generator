...
export class TradeModule {
  ...
  applyMarketPressure(
    ...
    return minmax(
      floor,
      price + units * basePrice * MARKET_PRESSURE_FACTOR,
      ceiling,
    );
  }

  private getMarketGood(
    market: Market,
    goodId: number,
    fallbackPrice = 0,
  ): MarketGoodData {
  ...
  recordDeal(data: Omit<Deal, "id">): Deal {
    ...
    if (taxAmount > 0 && stateId > 0)
      this.data.stateTaxes[stateId] =
        (this.data.stateTaxes[stateId] || 0) + taxAmount;
    return deal;
  }
  ...
  updateMarketDemand(
    ...
    for (const market of this.data.markets) {
      const aggregatedUncoveredDemand =
        this.createDemandVector(DEMAND_PRIORITY);
      ...
      for (const burgId of marketBurgIds) {
        ...
        const burgDemandTargets = DEMAND_PRIORITY.map(
          (category) =>
            (burg.population || 0) * DEMAND_TARGET_FACTORS[category],
        );
        const burgCoverage = this.calculateDemandCoverage(
          demandInventory.get(burgId) ?? {},
          this.goodById,
        );
      ...
      const demandTargets = aggregatedUncoveredDemand.map(
        (value) => value * (1 + TRADE_RESERVE_FACTOR),
      );
      const demandCoverage = this.calculateDemandCoverage(
        this.getMarketInventory(market),
        this.goodById,
      );
      void demandTargets.map((target, index) =>
  ...
  redistributeAcrossMarkets(
    ...
    for (const market of this.data.markets) {
      ...
      for (const burgId of marketBurgIds) {
        ...
        for (
          ...
          marketDemand[categoryIndex] += Math.max(
            0,
            burgDemandTargets[categoryIndex] - burgCoverage[categoryIndex],
          );
        }
      }
      const demandTargets = marketDemand.map(
        (value) => value * (1 + TRADE_RESERVE_FACTOR),
      );
      ...
      uncoveredDemandByMarket.set(
        market.i,
        demandTargets.map((target, index) =>
          Math.max(0, target - demandCoverage[index]),
        ),
      );
    ...
    for (const importer of this.data.markets) {
      const importerUncovered =
        uncoveredDemandByMarket.get(importer.i) ||
        this.createDemandVector(DEMAND_PRIORITY);
      for (
        let categoryIndex = 0;
        categoryIndex < DEMAND_PRIORITY.length;
            ...
            return Object.keys(exportPool)
              ...
              .flatMap((goodId) => {
                ...
                const coverageWeight =
                  good?.demandCoverage[demandCategory] || 0;
                if (!good || available <= 0.001 || coverageWeight <= 0)
                  return [];
        ...
        for (const candidate of candidates) {
          ...
          exporterPool[candidate.goodId] = Math.max(
            0,
            (exporterPool[candidate.goodId] || 0) - units,
          );
          const exporterGood = this.getMarketGood(
            candidate.exporter,
            candidate.goodId,
            candidate.good.value,
          );
          exporterGood.stock = Math.max(0, exporterGood.stock - units);
          const importerGood = this.getMarketGood(
            importer,
            candidate.goodId,
            candidate.good.value,
          );
          ...
          exporterGood.price = this.applyMarketPressure(
            candidate.good.value,
            exporterGood.price,
            -units,
          );
          importerGood.price = this.applyMarketPressure(
            candidate.good.value,
            importerGood.price,
            units,
          );
  ...
  buyFromMarket(params: {
    ...
    if (!market || params.units <= 0)
      return { units: 0, totalCost: 0, taxAmount: 0, dealId: null };
  ...
  sellToMarket(params: {
    ...
    const market = this.getMarketForBurg(burg);
    if (!market || params.units <= 0)
      return { units: 0, revenue: 0, taxAmount: 0, dealId: null };
  ...
  private buildMarkets(goods: Good[], burgs: Burg[]): Market[] {
    ...
    let minSpacing =
      (((graphWidth + graphHeight) * 4) / validBurgs.length ** 0.7) | 0;
  ...
  private splitInventoryByDemand(
    ...
    for (
      ...
      const candidates = Object.keys(excessInventory)
        ...
        .sort((a, b) => {
          const coverageA =
            goodById.get(a)?.demandCoverage[demandCategory] || 0;
          const coverageB =
            goodById.get(b)?.demandCoverage[demandCategory] || 0;
      ...
      for (const goodId of candidates) {
        const shortage = Math.max(
          0,
          demandTargets[categoryIndex] - retainedDemandCoverage[categoryIndex],
        );
        if (shortage <= 0.001) break;
        ...
        retainedInventory[goodId] =
          (retainedInventory[goodId] || 0) + keepAmount;
        excessInventory[goodId] = Math.max(0, available - keepAmount);

        for (
          let coverageCategoryIndex = 0;
          coverageCategoryIndex < DEMAND_PRIORITY.length;
          coverageCategoryIndex++
        ) {
          const coverageCategory = DEMAND_PRIORITY[
            coverageCategoryIndex
          ] as DemandCategory;
          const retainedCoverageWeight =
            good.demandCoverage[coverageCategory] || 0;
          if (!retainedCoverageWeight) continue;
          retainedDemandCoverage[coverageCategoryIndex] +=
            keepAmount * retainedCoverageWeight;
        }
      }
    }
  ...
  private seedMarketsFromRuralProduction(
    goods: Good[],
    markets: Market[],
  ): void {
    ...
    const marketsById = new Map<number, Market>(
      markets.map((market) => [market.i, market]),
    );
    ...
    for (const cellId of cells.i) {
      ...
      if (explicitGoodId) {
        const explicitGood = this.getMarketGood(
          market,
          explicitGoodId,
          this.goodById.get(explicitGoodId)?.value || 0,
        );
  ...
  private initializeMarketPrices(goods: Good[], markets: Market[]): void {
    ...
    for (const market of markets) {
      ...
      for (const good of goods) {
        ...
        const ratio =
          (demand + LAPLACE_PRICE_SMOOTHING) /
          (marketGood.stock + LAPLACE_PRICE_SMOOTHING);
        marketGood.price =
          good.value * minmax(ratio, PRICE_FLOOR_FACTOR, PRICE_CEILING_FACTOR);
      }
  ...
  private assignCellsToMarkets(markets: Market[]): void {
    ...
    for (const market of markets) {
      ...
      costs[startCell] = 1;
      queue.push(
        { cellId: startCell, marketId: market.i, burg, priority: 0 },
        0,
      );
    }
    ...
    while (queue.length) {
      ...
      for (const neighborId of cells.c[cellId]) {
        let cost = BASE_COST;
        if (cells.h[neighborId] < 20 && burg.port === cells.f[neighborId])
          cost -= SAME_PORT_WATERBODY_BONUS;
        if (Routes.areConnected(cellId, neighborId)) {
          cost -= SAME_ROUTE_BONUS;
        } else if (cells.h[neighborId] >= 70) {
          cost += MOUNTAIN_COST;
        }
        if (cellRiver && cellRiver === cells.r[neighborId])
          cost -= SAME_RIVER_BONUS;
        if (cellState && cellState !== cells.state[neighborId])
          cost += DIFFERENT_STATE_COST;
        if (cellProvince && cellProvince !== cells.province[neighborId])
          cost += DIFFERENT_PROVINCE_COST;
        ...
        if (!costs[neighborId] || totalCost < costs[neighborId]) {
          ...
          queue.push(
            { cellId: neighborId, marketId, burg, priority: totalCost },
            totalCost,
          );
  ...
  private collectConsumerDemand(goods: Good[]): number[] {
    ...
    for (const category of DEMAND_PRIORITY) {
      totalCoverageByCategory[category] =
        goods.reduce((sum, g) => sum + (g.demandCoverage[category] || 0), 0) ||
        1;
    }
    ...
    for (const good of goods) {
      demandFactor[good.i] = DEMAND_PRIORITY.reduce((sum, category) => {
        const share =
          (good.demandCoverage[category] || 0) /
          (totalCoverageByCategory[category] || 1);
  ...
  private collectIndustrialDemand(
    goods: Good[],
      ...
      for (const recipe of good.recipes) {
        for (const [ingredientIdStr, amount] of Object.entries(recipe)) {
          ...
          demandFactor[ingredientId] =
            (demandFactor[ingredientId] || 0) + amount * outputDemand;
        }
      }
    }
    return demandFactor;
