...
describe("ensureBurgLabelGroups", () => {
  ...
  it("adds registry entries for burg groups the label registry lacks", () => {
    ...
    const names = facts.labels.groups.filter(g => g.type === "burg").map(g => g.name);
    ...
    expect(facts.labels.groups.find(g => g.name === "city")?.zoom).toEqual({ min: 1.4, max: 25 });
    expect(facts.labels.groups.find(g => g.name === "customgroup")?.zoom).toEqual({ min: 2, max: 30 });
    // existing entries are left alone
    expect(facts.labels.groups.find(g => g.name === "cities")?.zoom).toEqual({ min: 1, max: 25 });
  });

  it("is idempotent", () => {
    labels.ensureBurgLabelGroups();
    const count = facts.labels.groups.length;
    labels.ensureBurgLabelGroups();
    expect(facts.labels.groups.length).toBe(count);
  });
});
