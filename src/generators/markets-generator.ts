...
export class MarketsModule {
  ...
  private createMarkets(): Market[] {
    ...
    let minSpacing = (((options.graph.width + options.graph.height) * 2) / pack.burgs.length ** 0.6) | 0;
  ...
  runGlobalTrade(): void {
    ...
    const mapDiagonal = Math.hypot(options.graph.width, options.graph.height) || 1;
