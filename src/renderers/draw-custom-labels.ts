...
import { CustomLabel } from "../modules/labels";
...
export function customLabelRenderer() {
  const customLabels = Labels.getAll().filter(labels => labels.type === "custom")
  const customLabelsHTML: string[] = [];
  const pathGroup = defs.select<SVGGElement>(
    "g#deftemp > g#textPaths",
  );
  for (const labelData of customLabels) {
    const pathId = addPathForLabel(labelData, pathGroup.node()!);
    console.log("Constructing label HTML for label", labelData, "with pathId", pathId);
...
function constructLabelHTML(label: CustomLabel, pathId: string): SVGTextElement {
  ...
  textPath.setAttribute("startOffset", label.startOffset ? `${label.startOffset}%` : "50%");
  textPath.setAttribute("font-size", label.fontSize ? `${label.fontSize}%` : "100%");
