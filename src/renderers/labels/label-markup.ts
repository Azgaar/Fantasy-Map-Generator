...
import type { LabelData, PathLabelData, PointLabelData } from "./draw-labels";
...
function getLabelTextMarkup(label: PathLabelData): string {
  ...
  return /*html*/ `<text text-rendering="optimizeSpeed" id="${label.id}" data-label-type="${label.type}" ${transform}>
      <textPath href="#textPath_${label.id}" startOffset="${startOffset}" font-size="${fontSize}"${letterSpacing}>${
