...
describe("selectRendererResolution", () => {
  it("caps device pixel ratio for ordinary desktop viewports", () => {
    expect(selectRendererResolution({ devicePixelRatio: 3, height: 720, width: 1280 })).toBe(2.5);
  });

  it("reduces resolution on constrained-memory devices", () => {
    expect(selectRendererResolution({ deviceMemoryGb: 4, devicePixelRatio: 2, height: 720, width: 1280 })).toBe(2);
    expect(selectRendererResolution({ deviceMemoryGb: 2, devicePixelRatio: 2, height: 720, width: 1280 })).toBe(1.5);
  });
  ...
  it("stays within the backing-canvas pixel budget", () => {
    ...
    expect(resolution).toBe(1.74);
    expect(3840 * 2160 * resolution ** 2).toBeLessThanOrEqual(24 * 1024 * 1024 * 1.01);
  });
