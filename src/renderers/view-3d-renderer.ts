...
async function createLabels() {
  ...
  for (let i = 1; i < pack.burgs.length; i++) {
    ...
    if (window.LayerControls.isLayerOn("toggleLabels") && !burg.label?.hidden) {
    ...
    // icons
    if (window.LayerControls.isLayerOn("toggleBurgIcons")) {
  ...
  // state labels
  if (window.LayerControls.isLayerOn("toggleLabels")) {
...
async function createMeshTextureUrl(): Promise<string> {
  const size = options.threeD.resolutionScale;
  const canvas = await Services.ExportMap.renderFullMapRaster(
    {
    ...
    { height: size, overlayType: "mesh", width: size }
  );
  let blob: Blob;
  try {
    blob = await new Promise<Blob>((resolve, reject) => {
      canvas.toBlob(
        value => (value ? resolve(value) : reject(new Error("Cannot render the 3D map texture"))),
        "image/png"
      );
    });
  } finally {
...
async function updateGlobeTexure(addMesh?: boolean) {
  ...
  // fill canvas segment with the complete Pixi map and the remaining SVG overlay
  const mapRaster = await Services.ExportMap.renderFullMapRaster(
    { noScaleBar: true, fullMap: true, noVignette: true },
    { height: Math.max(1, mapHeight), overlayType: "mesh", width: Math.max(1, mapWidth) }
  );
  ctx.drawImage(mapRaster, dx, dy, mapWidth, mapHeight);
  mapRaster.remove();
  if (texture) texture.dispose();
