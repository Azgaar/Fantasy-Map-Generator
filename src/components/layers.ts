...
const mapLayers = [
  ...
  new Layer({
    id: "routes",
    parent: "viewbox",
    // airroutes and traderoutes are fork groups: undeclared children are removed on erase
    children: ["roads", "trails", "searoutes", "airroutes", "traderoutes"].map(id => ({ id, tag: "g" })),
    draw: drawRoutes,
    erase: removeRoutes
  }),
