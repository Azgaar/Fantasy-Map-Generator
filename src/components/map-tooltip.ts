...
// Skip per-cell tooltip work while hovering inside the same cell/target. Pixel-by-pixel motion
// within one cell yields identical results, so cache the (cellId | gridCellId | targetId) tuple
// and short-circuit when unchanged.
let lastHoverStateKey = "";
...
export function handleMouseMove(event: MouseEvent | TouchEvent): void {
  ...
  const targetId = (event.target as Element | null)?.id ?? "";
  const stateKey = `${cellId}|${gridCellId}|${targetId}`;
  if (stateKey === lastHoverStateKey) return;
  lastHoverStateKey = stateKey;
