...
describe("WorkspaceToolbar", () => {
  test("renders Options, the map chooser, and an icon-only Preferences control", () => {
    const markup = renderToStaticMarkup(
      <WorkspaceToolbar initialMapSnapshot={snapshot} mapControls={controls} onOpenPreferences={vi.fn()} />
    );

    expect(markup.includes('id="workspaceOptionsTrigger"')).toBe(true);
    expect(markup.includes('aria-controls="workspaceOptionsMenu"')).toBe(true);
