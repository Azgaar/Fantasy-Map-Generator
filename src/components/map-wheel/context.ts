...
const DIRECT_MATCHERS: DirectMatcher[] = [
  {
    ...
    build: el => {
      ...
      const type = String(el.dataset.labelType) as LabelType;
      return {
        kind: "Label",
        ...
        actions: [
          {
            ...
            icon: "icon-edit",
            verb: "open",
            run: () => void Controllers.LabelsEditor.open(type, id)
          },
          {
            label: "All labels",
            icon: "icon-docs",
  ...
  return {
    ...
    actions: [
      ...
      { label: "All burgs", icon: "icon-docs", verb: "inspect", run: () => void Controllers.BurgsOverview.open() },
      {
        ...
        icon: "icon-coa",
        verb: "modify",
        run: () => void Controllers.EmblemsEditor.open("burg", `burgCOA${id}`, burg)
      }
    ]
  };
}
    ...
    add({
      ...
      actions: [
        ...
        { label: "Charts", icon: "icon-chart-pie", verb: "inspect", run: () => void Controllers.ChartsOverview.open() },
        {
          ...
          icon: "icon-coa",
          verb: "modify",
          run: () => void Controllers.EmblemsEditor.open("province", `provinceCOA${provinceId}`, province)
        }
      ]
    });
  }
    ...
    add({
      ...
      actions: [
        ...
        { label: "Charts", icon: "icon-chart-pie", verb: "inspect", run: () => void Controllers.ChartsOverview.open() },
        {
          ...
          icon: "icon-coa",
          verb: "modify",
          run: () => void Controllers.EmblemsEditor.open("state", `stateCOA${stateId}`, state)
        }
      ]
    });
  }
