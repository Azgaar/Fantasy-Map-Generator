...
const TEMPLATE = /* html */ `
  ...
  <table>
    ...
    <tr data-tip="Define a number of non-capital settlements to be placed (if enough suitable land exists)">
      ...
      <td>Burgs number</td>
      <td>
        <input id="manorsInput" data-option="manors" type="range" min="0" max="100001" step="1" value="100001" />
      </td>
      <td>
    ...
    >
      ...
      <td>
        <select id="viewportRedraw" data-option="viewportRedraw">
          <option value="continuous">While zooming</option>
          <option value="settled" selected>After zoom</option>
        </select>
      </td>
      <td></td>
    </tr>
...
export const cellsDensityColor = (cells: number): string =>
  cells > 200000 ? "#b12117" : cells > 50000 ? "#df6812" : cells === 10000 ? "#053305" : "#dfdf12";
