...
export class GoodsModule {
  ...
  getMethods(cellId: number = this.cellId) {
    return {
      ...
      habitability: () => biomesData.habitability[this.cells.biome[cellId]] > currentRNG() * 100,
...
export const GOODS: Record<number, { id: number; name: string; color: string; value: number }> = {};
for (const good of (Goods as any).defaultGoods) {
  GOODS[good.i] = { id: good.i, name: good.name, color: good.color, value: good.value };
}
